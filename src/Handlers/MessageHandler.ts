...
export default class MessageHandler {
  ...
  handleMessage = async (M: ISimplifiedMessage): Promise<void> => {
    ...
    if (!command)
      return void M.reply(
        `Ara ara, please use *${this.client.config.prefix}help* (✿≧_≦).`
      );
    const user = await this.client.getUser(M.sender.jid);
    if (user.ban) return void M.reply("YOU ARE BANNED BY THE OWNER (✿≧_≦)");
    const state = await this.client.DB.disabledcommands.findOne({
    ...
    if (
      ...
      return void M.reply(`ONLY MODS CAN USE THIS COMMAND! (✿≧_≦)`);
    }
    if (command.config?.adminOnly && !M.sender.isAdmin)
      return void M.reply(
        `THIS COMMAND IS ONLY FOR GROUP ADMINS!(✿≧_≦)`
      );
    try {
  ...
  sendReconnectMessage = async (): Promise<void> => {
    const text = `I'm back (✿≧_≦)`;
    await this.client.sendMessage(
      "919645689911@s.whatsapp.net",
      text,
      MessageType.text
    );
  };
